...
function DopeSheet() {
  ...
  }, [removeKeyframe, selectedClip, selectedKeyframe])
  ...
  const startKeyframeDrag = (event, propertyId, keyframeTime) => {
    ...
    setSelectedKeyframe({ propertyId, time: keyframeTime })
    setDragState({
  ...
  }, [isSameKeyframeTime, selectedClip, selectedKeyframe])

  useEffect(() => {
    if (!selectedKeyframe || dragState) return undefined
  ...
  }, [deleteSelectedKeyframe, dragState, selectedKeyframe])
    ...
    <div className="h-full bg-sf-dark-900 border-t border-sf-dark-700 flex flex-col min-h-0">
      <div className="h-8 px-3 border-b border-sf-dark-700 bg-sf-dark-800 flex items-center justify-between text-[11px]">
        <div className="text-sf-text-secondary flex items-center gap-3">
          ...
          <span className="text-[10px] text-sf-text-muted">Shift+Drag = move all properties at same time</span>
          {selectedKeyframe && (
            <button
              ...
              title="Delete selected keyframe (Delete / Backspace)"
            >
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        <div style={{ minWidth: `${LEFT_COLUMN_WIDTH + laneWidth}px` }}>
          <div className="flex h-8 border-b border-sf-dark-700 bg-sf-dark-800">
            ...
            <div
              ...
              onMouseDown={setPlayheadFromMouseEvent}
            >
                ...
                >
                  ...
                  {keyframes.map((keyframe, index) => (
                    <div
                      ...
                      title={`${property.label}: ${keyframe.value.toFixed(2)}${property.unit} at ${formatSeconds(keyframe.time)}`}
